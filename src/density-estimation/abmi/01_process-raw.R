...
# Pull select ABMI camera project IDs
abmi_proj_ids <- wt_get_download_summary(sensor_id = "CAM") |>
  # Remove CMU
  filter(!str_detect(project, "CMU")) |>
  # Included projects: EH, OG, OSM, Focal, Amphib, Edge-Interior, North Sask
  filter(str_detect(project, "Health|Off-Grid|ABMI OSM|Focal|Amphibian|Edge|Sask")) |>
  pull(project_id) |>
  unlist()

# Download data report using IDs - this takes ~ 20 minutes, depending on internet connection. No need to run every time.
data <- map_df(.x = cmu_proj_ids,
...
df_all_clean <- data |>
  ...
  select(project, location, date_detected, field_of_view, common_name, age_class, sex, number_individuals) |>
...
df_all_clean |>
  ...
  filter(common_name %in% native_sp) |>
  write_csv(paste0(g_drive, "data/base/clean/abmi_all-years_native-sp_clean_", Sys.Date(), ".csv"))

# Save (all) cleaned data
df_all_clean |>
  write_csv(paste0(g_drive, "data/base/clean/abmi_all-years_all-data_clean_", Sys.Date(), ".csv"))

# NONE gaps
df_gap_nones |>
  write_csv(paste0(g_drive, "data/processed/probabilistic-gaps/abmi_all-years_gap-class-nones_", Sys.Date(), ".csv"))
