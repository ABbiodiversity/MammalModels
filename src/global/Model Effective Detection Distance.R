...
# Lure lookup
df_lure <- read_csv(paste0(g_drive, "data/lookup/lure/abmi-cmu_all-years_lure_2021-06-21.csv"))
...
df_veghf_groups <- read_csv(paste0(g_drive,"data/lookup/veg-pole-distance.csv"))
...
# Save data
write_csv(df_pole_veghf, paste0(g_drive, "data/processed/detection-distance/clean-data/pole_position_veghf_", Sys.Date(), ".csv"))
...
for (sp in 1:length(SpTable)) {
  ...
  fname<-paste0(g_drive, "data/processed/detection-distance/figures/Detection distance ",SpTable[sp],".jpg",sep="")
  ...
  fname<-paste0(g_drive, "data/processed/detection-distance/group-models/Detection distance models ",SpTable[sp],".rdata",sep="")
...
fname.models <- paste0(g_drive, "data/processed/detection-distance/group-models/Detection distance models ")
...
load(paste0(g_drive, "data/processed/detection-distance/predictions/Detection distances by site species and season_2021-10-07.rdata"))

veg <- read_csv(paste0(g_drive, "data/lookup/veghf/abmi-cmu_all-years_veghf-soilhf-detdistveg_2021-10-06.csv")) |>
  select(1, 2, 5)

df_detdist_values <- dd |>
  as.data.frame() |>
  ...
  left_join(veg, by = c("location", "project")) |>
  select(dist_group, season, VegForDetectionDistance, detdist) |>
  distinct()
