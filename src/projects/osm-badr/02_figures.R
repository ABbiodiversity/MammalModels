...
on_off_jem <- jem_means |>
  ...
  select(vegetation, treatment, fine_scale, common_name, mean_density = density_adj) |>
...
on_off |>
  ...
  scale_y_sqrt(breaks = c(0, 1, 2, 3, 4, 5, 6), limits = c(0, 7)) +
  theme_classic() +
  labs(x = "Placement",
...
ggsave(paste0(g_drive, "results/osm/figures/wtd_on-off_sqrt.png"), height = 4, width = 8, dpi = 500)
...
on_off |>
  ...
  geom_point(data = (on_off_jem |> filter(common_name == "Moose")),
             ...
             position = position_jitterdodge(jitter.width = 0.2)) +
  scale_color_manual(values = veg.col) +
  scale_y_sqrt(breaks = c(0, 1, 2, 3, 4, 5, 6)) +
  theme_classic() +
  labs(x = "Placement",
       y = expression(Density~(individuals~per~km^2))) +
