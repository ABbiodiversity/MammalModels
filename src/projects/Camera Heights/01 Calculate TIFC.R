...
# Description: At each of the 68 paired sites with two cameras at different heights (1m vs 0.5m), estimate the total
...
# Let's briefly talk about the data that we have now for this project:

# 2022
# -- ABMI Height Comparison (20) - contains all the 0.5m cameras
# ----- OSM 2022 (10) - Note: all from 'A' (Low Disturbance) treatments
# ----- EH 2022  (10)
# -- ABMI OSM 2022 (10) - contains 1m cameras from OSM
# -- ABMI EH 2022 (10) - contains 1m cameras from EH

# 2023
# -- ABMI Height Comparison (30) - contains 0.5m cameras
# ---- OSM 2023 (30) - all from OSM
# -- ABMI OSM 2023 (30) - contains 1m cameras Note: Mix of treatments this time.
# -- Biodiversity Trajectories (40 - 20e) - contains both 1m and 0.5m

# Note: Talked to Brett B about moving the 0.5 deployments from Trajectories into Height Comparison 2023

# Total number of paired deployments so far: 70. Wow, pretty good!
...
proj <- wt_get_download_summary(sensor_id = "CAM") |>
  filter(str_detect(project, "Ecosystem Health 2022|Height|ABMI OSM 2022|Trajectories|ABMI OSM 2023")) |>
...
bdt_data <- data |>
  filter(str_detect(project, "Trajectories")) |>
  mutate(height = ifelse(str_detect(location, "-M$"), "half", "full"),
         location = str_remove(location, "-M$")) |>
  filter(location %in% bdt_loc)

data_all <- data |>
  filter(location %in% osm_eh_loc) |>
  mutate(height = ifelse(str_detect(project, "Height"), "half", "full")) |>
  bind_rows(bdt_data) |>
...
# Camera pairs to remove from analysis entirely:
...
# - (NEW) 1-1A3-CA3 -> Funkiness with the higher camera. Can't be sure it operated properly. Half seems fine.
# - (NEW) 1-2A3-CA2 -> Ditto to above. Half seems fine.
# - 14-1E1-CA6 -> full camera failed, remove location entirely (half is fine)
# - 9-1E2-CA1 -> full camera failed, remove location entirely (half is fine)

remove <- c("1-1A2-CA2", "1-1A2-CA3", "1-1A2-CA4", "1-1A2-CA5", "1-2A2-CA1",
            "1-1A3-CA3", "1-2A3-CA2",
            "14-1E1-CA6", "9-1E2-CA1")
...
# Height 2023 (0.5m cameras)
# 14-1A1-CA1 failed on 2023-10-24
# 9-1A1-CA2 failed on 2023-11-09
# 9-1C1-CA2 failed on 2023-09-18
# 9-2C1-CA2 failed on 2023-06-10 (full is fine)
# 9-2E1-CA7 failed on 2023-06-10

# OSM 2023
# 14-1C1-CA2 failed on 2023-10-08
# 14-1E2-CA1 failed on 2023-12-13
# 9-1E2-CA3 failed on 2023-06-10 (half is fine, can use as data point in modeling)
# 9-2C1-CA3 failed on 2023-09-24
...
data_subset <- data_all |>
  ...
  filter(!(image_date_time > as.Date("2021-08-07 00:00:00") & location == "793-NE"),
         ...
         # New
         !(image_date_time > as.Date("2023-10-24 00:00:00") & location == "14-1A1-CA1"),
         !(image_date_time > as.Date("2023-11-09 00:00:00") & location == "9-1A1-CA2"),
         !(image_date_time > as.Date("2023-09-18 00:00:00") & location == "9-1C1-CA2"),
         !(image_date_time > as.Date("2023-06-10 00:00:00") & location == "9-2C1-CA2"),
         !(image_date_time > as.Date("2023-06-10 00:00:00") & location == "9-2E1-CA7"),
         !(image_date_time > as.Date("2023-10-08 00:00:00") & location == "14-1C1-CA2"),
         !(image_date_time > as.Date("2023-12-13 00:00:00") & location == "14-1E2-CA1"),
         !(image_date_time > as.Date("2023-06-10 00:00:00") & location == "9-1E2-CA3"),
         !(image_date_time > as.Date("2023-09-24 00:00:00") & location == "9-2C1-CA3")) |>
  ...
  # Turn 'Deer' into 'White-tailed Deer'
