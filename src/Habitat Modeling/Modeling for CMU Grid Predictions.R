...
# Title:            Re-estimate models for predicting to CMU grids
# Description:
...
g_drive <- "G:/Shared drives/ABMI Camera Mammals/Results/Habitat Models/2022/Analysis North for CMU Grids/"
...
fname.km2summaries <- paste0(g_drive, "km2 Summaries/Best Model/Grid summary") # Start of file name for km2 grid reference and current output for each species
...
# ABMI OSM 2021 - Remove everything except for LU 8
# Focal Areas 2019 and 2020 - Remove both
# All CMU
# Selections from ABMI EH 2017-2019

d_new <- d |>
  filter(!str_detect(project, "Focal Areas|CMU"),
         !(str_detect(location, "^2-|^3-|^4") & project == "ABMI OSM 2021"),
         !(str_detect(location, "602|603|604|634|635|636|666|667|668|826|827|828|858|859|860") & project == "ABMI Ecosystem Health 2017"),
         !(str_detect(location, "605|606|607|637|638|639|669|670|671|695|696|727|758") & project == "ABMI Ecosystem Health 2018"),
         !(str_detect(location, "510|511|540|541|542|571|572|573|790|821|854")))
...
# Just missing NWSAR and BG. I think that's fine. Not very reliable anyway.
...
# New SpTable - Just the critters we're interested in for this application
SpTable <- c("Moose", "Coyote", "White-tailed Deer", "Woodland Caribou", "Canada Lynx",
             "Gray Wolf", "Marten", "Fisher")
...
# NEW kgrid info from Brandon

# First, the grid lookup so we know which cells are in which grids
load("S:/samba/abmisc/ToMarcus/CMUGrid/results/kgrid-lookup.Rdata")
grid_linkid <- lookup.table

# This is the new summary ...
load("S:/samba/abmisc/ToMarcus/CMUGrid/results/backfillV7_cmu-grid_2021HFI.Rdata")
grid_currentveg <- as.data.frame(as.matrix(d.wide$veg.current)) |>
  rownames_to_column(var = "LinkID")

# Load previously-processed km2 file
load(km2file.out)
...
km2 <- kgrid_veg_current_north |>
  semi_join(grid_linkid)
km2.b <- kgrid_veg_reference_north |>
  semi_join(grid_linkid)
rm(kgrid_veg_current_north, kgrid_veg_reference_north)

# Okay, let's "swap" in the new Veg summary information
km2_new <- km2 |>
  # Remove old VegHF
  select(1:38, Intercept) |>
  # Join new VegHF summary information from Brandon
  left_join(grid_currentveg) |>
  mutate(cell_area = rowSums(across(SpruceR:CCPine4))) |>
  # Convert each veghf area to proportion of the cell area
  mutate(across(SpruceR:CCPine4, ~ .x / cell_area)) |>
  ...
  # Add some missing NSR
  mutate(NSR1Parkland = 0,
         NSR1Mountain = 0,
         NSR1DryMixedwood = 0,
         NSR1Shield = 0)

# Group HF types
df_hf <- read_csv("G:/Shared drives/ABMI Camera Mammals/data/lookup/locations/lookup-hf-class.csv") |>
  # Only interested in the HF types present in the kgrid data
  filter(HF_GROUP %in% colnames(km2_new))
...
for (i in groups) {
  ...
  if (i == "HFor") {
    hf_types_in_group <- str_subset(colnames(km2_new), pattern = "^CC")
  }
...
# Okay, that is all well and good for the km2 predictions. We also want to predict at the grid scale, though.

# Climate variables for these grids:

grids_nsr <- km2_new |>
  left_join(grid_linkid) |>
  select(CMUGrid, NR, NSR, LUF) |>
  distinct() |>
  group_by(CMUGrid) |>
  # This works out well — LLB becomes Lower Boreal Highlands and PEL becomes Central Mixedwood (both in the majority of cells)
  filter(row_number() == 1) |>
  ungroup()

grids_climate <- km2_new |>
  select(LinkID:HTV, Intercept) |>
  select(-c(NR, NSR, LUF)) |>
  left_join(grid_linkid) |>
  group_by(CMUGrid) |>
  summarise(across(Long:HTV, mean)) |>
  left_join(grids_nsr)

grids <- grid_currentveg |>
  pivot_longer(cols = SpruceR:CCPine4, names_to = "Veg", values_to = "area") |>
  left_join(grid_linkid) |>
  group_by(CMUGrid, Veg, .drop = FALSE) |>
  summarise(total_area = sum(area)) |>
  ungroup() |>
  pivot_wider(names_from = Veg, values_from = total_area) |>
  mutate(grid_area = rowSums(across(Bare:WindGenerationFacility))) |>
  # Convert each veghf area to proportion of the cell area
  mutate(across(Bare:WindGenerationFacility, ~ .x / grid_area)) |>
  # Combine veg categories in the km2 grid to match available coefficients
  mutate(TreedSwamp = rowSums(across(TreedSwamp1:TreedSwamp9)),
         TreedFen = rowSums(across(TreedFen1:TreedFen9)),
         TreedShrubSwamp = TreedSwamp + ShrubbySwamp,
  ...
  mutate(value = 1) |>
  pivot_wider(names_from = NSR_ALT, values_from = value, values_fill = list(value = 0)) |>
  # Add some missing NSR just so that the scripts below don't cry
  mutate(NSR1Parkland = 0,
         NSR1Mountain = 0,
         NSR1DryMixedwood = 0,
         NSR1Shield = 0,
...
df_hf <- read_csv(paste0(g_drive, "data/lookup/locations/lookup-hf-class.csv")) |>
  # Only interested in the HF types present in the kgrid data
  filter(HF_GROUP %in% colnames(grids))
...
for (i in groups) {
  ...
  # Add the individual cutblock ('CC') types to the HF group
  if (i == "HFor") {
    hf_types_in_group <- str_subset(colnames(grids), pattern = "^CC")
  }
...
# Okay, now the grids are ready to be used in the loops below.
...
km2.sc<-data.frame(grids[,c("Intercept","Lat","Long","TrueLat", "Easting", "Northing",
                   ...
                   Lat3=grids$Lat^3,
                   Long2=grids$Long^2,
                   LatLong=grids$Lat*grids$Long,
                   Lat2Long2=grids$Lat^2*grids$Long^2,
                   # Add the Easting/Northing ones
                   EN = grids$Easting * grids$Northing,
                   E2 = grids$Easting^2,
                   N2 = grids$Northing^2,
                   LongMAT=grids$Long*grids$MAT,
                   #MAPPET=grids$MAP*grids$PET,
                   MATAHM=grids$MAT*grids$AHM,
                   MAPFFP=grids$MAP*grids$FFP,
                   MAT2=grids$MAT*(grids$MAT+10),
                   MWMT2=grids$MWMT^2)
...
jpeg(file="G:/Shared drives/ABMI Camera Mammals/Results/Habitat Models/2022/Analysis North for CMU Grids/Maps/Spatial projection area NORTH.jpg",height=600,width=400)
...
km2.1<-grids #km2  # Version to use if no non-plottable region
km2.b.1<-grids #km2.b  # Version to use if no non-plottable region
...
vnames.sc<-c("Intercept","Lat","Long","LatLong","Lat2","Lat3","Long2","Lat2Long2",
             "MAT", "FFP", "TD", "MAT", "FFP", "PAS", "CMD", "AHM", "SHM", "Eref", "MAP", "MSP",
...
# This loops through each species in SpTable (8)
...
for (sp in 1:length(SpTable)) {
  ...
  if (sc.option[sp]==2 | sc.option[sp]==1) { # Presence/absence model only.  It is run for option 1 here solely to do fit AUC; the total abundance models replace these in the next section for option 1.
    ...
    m.sc.pa[[8]] <- try(update(m.sc.pa[[1]], .~. + MSP + PAS))
    ...
    m.sc.pa[[13]] <- try(update(m.sc.pa[[1]], .~. + MAT + FFP + TD + MSP + PAS))
    m.sc.pa[[14]] <- try(update(m.sc.pa[[1]], .~. + MSP + PAS + CMD + Eref))
    ...
    m.sc.pa[[18]] <- try(update(m.sc.pa[[1]], .~. + Easting + Northing + Easting * Northing + I(Easting^2) + I(Northing^2)))
  ...
  vnames1<-ifelse(vnames1=="Easting:Northing", "EN", vnames1)
  vnames1<-ifelse(vnames1=="I(Easting^2)", "E2", vnames1)
  vnames1<-ifelse(vnames1=="I(Northing^2)", "N2", vnames1)
  ...
  if (sc.option[sp]==1 | sc.option[sp]==2) {
    ...
    vnames<-ifelse(vnames=="Easting:Northing", "EN", vnames)
    vnames<-ifelse(vnames=="I(Easting^2)", "E2", vnames)
    vnames<-ifelse(vnames=="I(Northing^2)", "N2", vnames)
    vnames<-gsub(":","",vnames)
  ...
  q<-data.frame(Grid=km2.1$CMUGrid,Curr=km2.p) # Ref=km2.p.ref,
  ...
  save(file=fname,aic.wt.pa.save,
       aic.wt.agp.save,
       aic.age,
       aic.wt.age.models.save,
       aic.wt.age.save)
...
# Turn into one grid summaries file

wtd <- read_csv(paste0(g_drive, "km2 Summaries/Best Model/Grid summary White-tailed Deer.csv")) |>
                  mutate(common_name = "White-tailed Deer")

moose <- read_csv(paste0(g_drive, "km2 Summaries/Best Model/Grid summary Moose.csv")) |>
  mutate(common_name = "Moose")

caribou <- read_csv(paste0(g_drive, "km2 Summaries/Best Model/Grid summary Woodland Caribou.csv")) |>
  mutate(common_name = "Woodland Caribou")

blackbear <- read_csv(paste0(g_drive, "km2 Summaries/Best Model/Grid summary Black Bear.csv")) |>
  mutate(common_name = "Black Bear")

Coyote <- read_csv(paste0(g_drive, "km2 Summaries/Best Model/Grid summary Coyote.csv")) |>
  mutate(common_name = "Coyote")

Fisher <- read_csv(paste0(g_drive, "km2 Summaries/Best Model/Grid summary Fisher.csv")) |>
  mutate(common_name = "Fisher")

Marten <- read_csv(paste0(g_drive, "km2 Summaries/Best Model/Grid summary Marten.csv")) |>
  mutate(common_name = "Marten")

Lynx <- read_csv(paste0(g_drive, "km2 Summaries/Best Model/Grid summary Canada Lynx.csv")) |>
  mutate(common_name = "Canada Lynx")

Wolf <- read_csv(paste0(g_drive, "km2 Summaries/Best Model/Grid summary Gray Wolf.csv")) |>
  mutate(common_name = "Gray Wolf")

all <- bind_rows(
  wtd, moose, caribou, blackbear, Coyote, Fisher, Marten, Lynx, Wolf) |>
  rename(Prediction = Curr) |>
  select(Grid, common_name, Prediction)

write_csv(all, paste0(g_drive, "km2 Summaries/Best Model/Grid summary All Species.csv"))
