...
# Includes models of additional effect of 150m veg+HF
# Data has already been processed to density (/km2)
# Run step 1 first to set up the data files.  Also, run separate script for province-wide km2 processing whenever that changes.
...
# Set up file names for various outputs - need to make all these subdirectories!
fname.fig<-"C:/Dave/ABMI/Cameras/Coefficients/2021/Analysis north/Figures/Best model/"  # Start of figure file, one per species for veg+HF.  Subdirectory, species name and extension will be added
fname.map<-"C:/Dave/ABMI/Cameras/Coefficients/2021/Analysis north/Maps/"  # Start of map file, one per species for, spatial+climate map, current, reference and difference maps.  Subdirectory, species name and extension will be added.  Note: the web uses only whole-province maps, so naming here can be non-official
fname.sumout<-"C:/Dave/ABMI/Cameras/Coefficients/2021/Analysis north/Coefficient tables/Mammal coefficients North Nov 2021 Best model"  # For exporting coefficient tables
fname.Robjects<-"C:/Dave/ABMI/Cameras/Coefficients/2021/Analysis north/R objects/R objects North mammal coefficients Nov 2021 Best model"  # Start of file name to save each species' models
fname.Robjects.sum<-"C:/Dave/ABMI/Cameras/Coefficients/2021/Analysis north/R objects/R objects North mammals Coefficient tables Nov 2021 Best model.Rdata"
fname.km2summaries<-"C:/Dave/ABMI/Cameras/Coefficients/2021/Analysis north/Km2 summaries/Best model/Km2 North reference and current Nov 2021" # Start of file name for km2 grid reference and current output for each species
fname.useavail<-"C:/Dave/ABMI/Cameras/Coefficients/2021/Analysis north/Figures/Use availability/"  # Subdirectory for use-availability figures and table
km2file.out<-"C:/Dave/ABMI/Data/Km2 grid/2020/R km2 grid current and backfilled processed 2016 NORTH June 2020.Rdata"    # File with processed km2 grid files - processed in separate script.  Despite the variable name, this is an input here.
fname.150mVegModels<-"C:/Dave/ABMI/Cameras/Coefficients/2021/Analysis north/R objects/Veg 150m best model"  # Start of file name to save best model using 150m veg as well as that model combined with best space/climate model (for separate graphing)

# File name for previously processed datafile+veg+HF, km2 grid info, SpTable, and look-up matrix for veg groups
dataset.out<-"C:/Dave/ABMI/Cameras/Coefficients/2021/Analysis north/R Dataset SpTable for ABMI North mammal coefficients 2021.RData"
load(dataset.out)
SpTable.w<-SpTable.w[SpTable.w!="BlackBearWinter"]  # Bears have enough occurrences in winter to meet criterion, but they are just flukey encounters - don't run winter models
...
taxa.file<-read.csv("C:/Dave/ABMI/Data/Mammals/Mammal taxa.csv")  # To get proper species names for figures, tables
...
load(km2file.out)  # km2 for current veg+HF, km2.b for backfilled veg
km2$Intercept<-1
km2.b$Intercept<-1
...
# For each species, point veg+HF models are fit for summer and winter (and best-model coefficient figures plotted), then results are combined (and full coefficient figures plotted), and space/climate residual models are fit (and maps are plotted)
for (sp in 1:length(SpTable)) {
  ...
  for (seas in 1:2) {  # 1=Summer, 2=Winter
    ...
    if ( (seas==1 & SpSeas%in%SpTable.s==TRUE) | (seas==2 & SpSeas%in%SpTable.w==TRUE) ) {  # Only run if that species is in the species table for that season
      ...
      lure1<-mean(d.sp$Count[UseForLure==TRUE & d.sp$Lured=="Yes"])/mean(d.sp$Count[UseForLure==TRUE & d.sp$Lured=="No"])  # Lure effect on total abundance
      ...
      # 1.1 Fit point veg+HF models for presence/absence
      m.pa<-list(NULL)
  ...
  dplot(1:nrow(d),match(d$location_project,d1$location_project),cex=0.3)  # Making sure that deployments in d and d1 are the same and in same order before cbind'ing.  Needs to be a straight 1:1 line
...
lu.names<-read.csv("C:/Dave/ABMI/Cameras/Coefficients/2020/Analysis north/Lookup for coefficient table names North June 2020.csv")  # To translate names used for coefficients here to official names.  This seems to change several times a year - never know what is current.
...
write.table(q,file="C:/Dave/ABMI/Cameras/Coefficients/2021/Analysis north/AUC of fit for camera mammals North Nov 2021.csv",sep=",",row.names=F)

# Lure effects
fname<-"C:/Dave/ABMI/Cameras/Coefficients/2021/Analysis north/Lure effects North.csv"
...
fname<-"C:/Dave/ABMI/Cameras/Coefficients/2021/Analysis north/Model weights Veg+HF North.csv"
...
# Age Model weights
fname<-"C:/Dave/ABMI/Cameras/Coefficients/2021/Analysis north/Model weights Age stand groupings.csv"
...
write.table(q,file=fname,sep=",",row.names=FALSE)
fname<-"C:/Dave/ABMI/Cameras/Coefficients/2021/Analysis north/Model weights Age spline versus null.csv"
